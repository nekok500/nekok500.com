...
import { ColHTMLAttributes, useState } from "react";
...
function discordCDNLoader(props: ImageLoaderProps): string {
  return `https://cdn.discordapp.com/${props.src}?size=${props.width}`;
}

function Collaborator({ name, avatar }: { name: string; avatar: string }) {
  const [isPopoverOpen, setIsPopoverOpen] = useState(false);

  return (
    <div className="mx-auto flex justify-center mx-1">
      <Popover
        ...
        content={
          <p className="p-2 bg-white border border-gray-200 rounded-lg">
            {name}
          </p>
        }
      >
...
function ProjectCard({
  ...
  collab?: Array<React.ReactNode>;
  children: React.ReactNode;
  link?: string;
}) {
  return (
    <a href={link}>
      <div className="m-2 p-3 bg-white shadow my-2 border border-gray-200 rounded-lg">
        <h5 className="text-xl font-medium leading-tight text-gray-900 flex items-center">
          {title}
        </h5>
    ...
    <div className="mx-auto py-6">
      ...
      <div className="my-3 flex mx-auto justify-center text-4xl">
        ...
        <Popover
          ...
          positions={["top"]}
          reposition={false}
          onClickOutside={() => setIsPopoverOpen(false)} // preferred positions by priority
          content={
            <div className="p-1 flex items-center justify-between inline-block transition-opacity duration-300 bg-white border border-gray-200 rounded-lg">
              <Image
                className="rounded-full"
                src={
                  "avatars/586157827400400907/f6f111c136d36b14bc7ca42738ce9ebb.webp"
                }
                loader={discordCDNLoader}
                width={40}
                height={40}
                alt={""}
              />
              <div className="pl-1 flex flex-col items-center justify-between leading-4">
                <p className="font-semibold text-lg text-gray-900">
                  ねこかわいい
                </p>
                <p className="font-sm text-gray-600">@nekok500</p>
              </div>
            </div>
          }
        >
      ...
      <h2 className="mt-8 text-3xl font-bold text-gray-900">プロジェクト</h2>
      <div className="flex flex-col border-t-2">
        <ProjectCard
          title={[<FaPython />, "Kuroneko-TTSBot"]}
          collab={[
            <Collaborator
              name="黒猫ちゃん"
              avatar="608788412367110149/5f38a5151d88a13e825ab6c785774253.webp"
            />,
            <Collaborator
              name="ap12"
        ...
        >
          ...
          <p className="text-gray-600">discord.py, MongoDB</p>
        </ProjectCard>
        <ProjectCard
          title={[
            <svg
        ...
        >
          ...
          <p className="text-gray-600">Gin, discord.go</p>
          <p className="text-gray-600">Pillow</p>
        </ProjectCard>
      </div>
    </div>
  );
